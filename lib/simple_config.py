...
SYSTEM_CONFIG_PATH = "/etc/electrum-zaap.conf"
...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electrum-zaap directory", path)
        return path
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-zaap.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-zaap.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-zaap.conf into user_config[]."""
    if not path:
        return {}
