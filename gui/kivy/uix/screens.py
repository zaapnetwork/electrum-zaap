...
from electrum_zaap.util import (profiler, parse_URI, format_time,
                                InvalidPassword, NotEnoughFunds)
from electrum_zaap import bitcoin
from electrum_zaap.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        ...
        try:
            uri = electrum_zaap.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a zaap URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_zaap.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a zaap address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid zaap Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_zaap.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share zaap Request"))
