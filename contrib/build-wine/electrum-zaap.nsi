...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-zaap"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-zaap-${PRODUCT_VERSION}-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-zaap.ico"
...
Section
  ...
  File /r "dist\electrum-zaap\*.*"
  File "icons\electrum-zaap.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-zaap-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-zaap-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-zaap-${PRODUCT_VERSION}.exe" 0

  ;Links zaap: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\zaap" "" "URL:zaap Protocol"
  WriteRegStr HKCU "Software\Classes\zaap" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\zaap" "DefaultIcon" "$\"$INSTDIR\electrum-zaap.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\zaap\shell\open\command" "" "$\"$INSTDIR\electrum-zaap-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-zaap.ico"
