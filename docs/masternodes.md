...
Electrum-zaap supports masternode creation through an interface called the Masternode Manager.
...
- The zaap address that received the collateral payment.
- The zaap address of the delegate key.
...
A masternode requires a "delegate" key, which is known to both Electrum-zaap and your masternode.
Your masternode will use this key to sign messages, and the zaap network will know that you authorized
it to.

A delegate key can either be one of your Electrum-zaap keys, or an imported key. Either way, your masternode
and Electrum-zaap will both need to know the private key. (See *Importing Masternode.conf* below.)

To use one of your Electrum-zaap keys as a delegate key, put its address in the `Masternode zaap Address`
...
### Collateral (1000 zaap Payment)

To start a masternode, you must have a 1000 zaap payment available in your wallet.
You can scan your wallet for 1000 zaap payments in the `Activate Masternode` tab of the Masternode
Manager.

After scanning, a list of available 1000 zaap collateral payments will be displayed. Selecting one
...
waiting for Electrum-zaap to sign and broadcast your masternode announcement, you will be presented with
